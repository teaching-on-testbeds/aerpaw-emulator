...
from aerpawlib.runner import StateMachine, state, timed_state
...
class RoverSearch(StateMachine):
    ...
    def initialize_args(self, extra_args: list[str]):
        ...
        defaultFile = "ROVER_SEARCH_DATA_%s.csv" % datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    ...
    async def take_measurement(self, vehicle: Drone):
        ...
        if self.save_csv:
            ...
            self.csv_writer.writerow([position.lon, position.lat, position.alt, measurement])
    ...
    async def end(self, vehicle: Drone):
        ...
        print(f"Search time of {self.search_time} minutes has elapsed. Returning to launch!")
        print(f"Best rover location estimate {self.best_pos.lat, self.best_pos.lon} with measurement {self.best_measurement} after {datetime.datetime.now() - self.start_time} minutes")
