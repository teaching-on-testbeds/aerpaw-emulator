...
FILTER_ALLOWED_COPTER = "allowed-copter.jsonc"
FILTER_ALLOWED_ROVER = "allowed-rover.jsonc"
...
if __name__ == "__main__":
    parser = ArgumentParser(description="run-safety-checker-sitl-tests - trigger tooling to spawn sitl and a vehicle script under screen")
    parser.add_argument("--sitl_instance", required=False, default="1", dest="sitl_instance")
    parser.add_argument("--script_params", required=False, default=False, dest="script_params")
    ...
    params_file = open(args.geofence_params)
    ...
    os.system(f"screen -L -Logfile screenlog_safetychecker -S {SCREEN_PREFIX}_{SCREEN_SAFETY_CHECKER_PREFIX} -dm bash -c '{safety_checker_cmd}'")
    ...
    if not args.no_script:
        ...
        os.system(f"screen -L -Logfile screenlog_vehiclescript -S {SCREEN_PREFIX}_{SCREEN_VEHICLE_SCRIPT_PREFIX} -dm bash -c '{script_cmd}'")
    ...
    if args.no_script:
        print(f"No vehicle script launched. Manually launch with address udp:127.0.0.1:{VEHICLE_SCRIPT_PORT}")

    exit_commands = [f"screen -S {SCREEN_PREFIX}_{service} -X quit" for service in launched_services]
    # print(exit_commands)
