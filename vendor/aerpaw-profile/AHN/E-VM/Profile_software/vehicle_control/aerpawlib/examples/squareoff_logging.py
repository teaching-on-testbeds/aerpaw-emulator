...
from aerpawlib.runner import StateMachine, background, state, timed_state
...
class SquareOff(StateMachine):
    ...
    def initialize_args(self, extra_args: list[str]):
        ...
        parser.add_argument("--samplerate", help="log sampling rate (Hz)", required=False, default=1, type=float)
    ...
    async def command_leg(self, vehicle: Vehicle, dNorth: float, dEast: float):
        ...
        await vehicle.goto_coordinates(vehicle.position + VectorNED(dNorth, dEast), tolerance=LOCATION_TOLERANCE)
