...
class CSwSNRTX(gr.top_block):
    def __init__(self, args="", channel=0, freq=3.32e9, gaintx=76, offset=250e3, samp_rate=2e6, sps=16):
...
def main(top_block_cls=CSwSNRTX, options=None):
    ...
    tb = top_block_cls(args=options.args, channel=options.channel, freq=options.freq, gaintx=options.gaintx, offset=options.offset, samp_rate=options.samp_rate, sps=options.sps)
