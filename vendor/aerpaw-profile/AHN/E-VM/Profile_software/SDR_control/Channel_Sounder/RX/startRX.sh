...
screen -S sounderRXGRC -dm bash -c "python3 sounder_RX.py -g $GAIN -r $RATE -f $FREQUENCY --arguments $ARGS  --mode $mode --ip $IP"

screen -S sounderRXoctave -dm bash -c "octave sounder_RX.m $GAIN | ts '[%Y-%m-%d %H:%M:%.S]' |tee sounderRX-octave`date +%F-%T`.log"
