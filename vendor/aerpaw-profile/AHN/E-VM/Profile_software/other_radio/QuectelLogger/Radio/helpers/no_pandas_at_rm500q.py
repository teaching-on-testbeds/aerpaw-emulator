...
if __name__ == "__main__":
    ...
    class RM500Q(ATProtocol):
        ...
        def command_with_event_response(self, command, outresponse="OK", timeout=2):
            ...
            with self.lock:  # ensure that just one thread is sending commands at once
                ...
                # self.transport.write(bytes(_cmd, 'utf-8') + b'\n')
                # print(self.transport.write(command.encode(self.ENCODING, self.UNICODE_HANDLING) + b'\r\n' ))
                # print("WRITE",self.transport.write("AT\r\n".encode(self.ENCODING, self.UNICODE_HANDLING)))
                print(ser.write(command.encode(self.ENCODING, self.UNICODE_HANDLING) + b"\r\n"))
                # ser.write(b"ATI\r\n")
                Real_response = []
                while True:
                    try:
                        ...
                        time.sleep(0.1)
                        response = ser.read(size=64).decode("utf-8")
                        print("OK", response)
    ...
    if continue_execution:
        try:
            ...
            ser = serial.Serial("/dev/ttyUSB3", baudrate=115200, bytesize=8, timeout=1)
        except:
            continue_execution = True
            pass
    ...
    if continue_execution:
        try:
            ser = serial.serial_for_url("/dev/ttyUSB4", baudrate=115200, timeout=1, stopbits=1, rtscts=1)
        except:
