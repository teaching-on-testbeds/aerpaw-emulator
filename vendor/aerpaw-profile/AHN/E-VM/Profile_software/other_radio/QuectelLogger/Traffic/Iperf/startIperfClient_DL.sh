...
#
#Example: ./startIperfClient_DL.sh <Source IP> <Destination IP> <port def 5201> 
#
log_file="Iperf_client_DL_log_$(date +"%Y_%m_%d_%H\:%M\:%S").log"
...
#screen -S Iperf -dm bash -c "stdbuf -oL -eL iperf3 -c $2 -t $IPERF_DURATION -B $1 -p $3 -R \
#	| tee log/$log_file"

bash -c "stdbuf -oL -eL iperf3 -c $2 -t $IPERF_DURATION -B $1 -p $3 -R \
        | ts | tee log/$log_file"
