...
from aerpawlib.util import Coordinate, doIntersect, inside, readGeofence
...
def serialize_response(request_function: str, result: bool, message: str = ""):
    raw_msg = json.dumps({"request_function": request_function, "result": result, "message": message})
    return serialize_msg(raw_msg)
...
class SafetyCheckerClient:
    ...
    def validateWaypointCommand(self, curLoc: Coordinate, nextLoc: Coordinate):
        ...
        """
        msg = serialize_request(VALIDATE_WAYPOINT_REQ, [curLoc.toJson(), nextLoc.toJson()])
        resp = self.sendRequest(msg)
        return self.parseResponse(resp)
    ...
    def validateTakeoffCommand(self, takeoffAlt, currentLat, currentLon):
        ...
        """
        msg = serialize_request(VALIDATE_TAKEOFF_REQ, [takeoffAlt, currentLat, currentLon])
        resp = self.sendRequest(msg)
        return self.parseResponse(resp)
...
class SafetyCheckerServer:
    ...
    def __init__(self, vehicle_config_filename: str, server_port=14580):
        ...
        vehicle_config_file = open(vehicle_config_filename)
    ...
    def validate_config(self, config: dict, vehicle_config_filename: str):
        ...
        for param in self.REQUIRED_PARAMS:
            if param not in config:
                raise Exception(f"Required parameter {param} not found in {vehicle_config_filename}!")

        # Ensure the vehicle type is valid
        if config["vehicle_type"] not in self.VEHICLE_TYPES:
            raise Exception(f"Vehicle type in {vehicle_config_filename} is invalid! Must be one of {self.VEHICLE_TYPES}")
        ...
        if config["vehicle_type"] == "copter":
            for param in self.REQUIRED_COPTER_PARAMS:
                if param not in config:
                    raise Exception(f"Required copter parameter {param} not found in {vehicle_config_filename}!")
    ...
    def validateWaypointCommand(self, curLoc: Coordinate, nextLoc: Coordinate) -> tuple[bool, str]:
        ...
        if self.vehicle_type == "copter":
            if nextLoc.alt < self.min_alt or nextLoc.alt > self.max_alt:
                return (
                    False,
                    "Invalid waypoint. Altitude of %s m is not within restrictions! ABORTING!" % nextLoc.alt,
                )
        ...
        if inside_geofence == False:
            return (
                False,
                "Invalid waypoint. Waypoint (%s,%s) is outside of the geofence. ABORTING!" % (nextLoc.lat, nextLoc.lon),
            )
        ...
        for zone in self.exclude_geofences:
            if inside(nextLoc.lon, nextLoc.lat, zone):
                return (
                    False,
                    "Invalid waypoint. Waypoint (%s,%s) is inside a no-go zone. ABORTING!" % (nextLoc.lat, nextLoc.lon),
                )
        ...
        for i in range(len(geofence) - 1):
            if doIntersect(
                ...
                return (
                    False,
                    "Invalid waypoint. Path from (%s,%s) to waypoint (%s,%s) leaves geofence. ABORTING!" % (curLoc.lat, curLoc.lon, nextLoc.lat, nextLoc.lon),
                )
        ...
        for zone in self.exclude_geofences:
            for i in range(len(zone) - 1):
                if doIntersect(
                    ...
                    return (
                        False,
                        "Invalid waypoint. Path from (%s,%s) to waypoint (%s,%s) enters no-go zone. ABORTING!" % (curLoc.lat, curLoc.lon, nextLoc.lat, nextLoc.lon),
                    )
    ...
    def validateChangeSpeedCommand(self, newSpeed) -> tuple[bool, str]:
        ...
        if newSpeed > self.max_speed:
            return (
                False,
                "Invalid speed (%s) greater than maximum (%s)" % (newSpeed, self.max_speed),
            )
        if newSpeed < self.min_speed:
            return (
                False,
                "Invalid speed (%s) less than minimum (%s)" % (newSpeed, self.min_speed),
            )
        # New speed is valid
        return (True, "")
    ...
    def validateLandingCommand(self, currentLat, currentLon):
        ...
        if distance > 5:
            return (
                False,
                "Invalid landing location. Must be within 5 meters of takeoff location. Attempted landing location (%s,%s) is %f meters from takeoff location." % (currentLat, currentLon, distance),
            )
    ...
    def validateWaypointHandler(self, curLocJSON, nextLocJSON, *_params):
        ...
        curLoc = Coordinate(lat=curLocDict["lat"], lon=curLocDict["lon"], alt=curLocDict["alt"])
        nextLoc = Coordinate(lat=nextLocDict["lat"], lon=nextLocDict["lon"], alt=nextLocDict["alt"])

        result, message = self.validateWaypointCommand(curLoc, nextLoc)
        msg = serialize_response(request_function=VALIDATE_WAYPOINT_REQ, result=result, message=message)
        return msg

    def validateChangeSpeedHandler(self, newSpeed, *_params):
        result, message = self.validateChangeSpeedCommand(newSpeed)
        msg = serialize_response(request_function=VALIDATE_CHANGE_SPEED_REQ, result=result, message=message)
        return msg

    def validateTakeoffHandler(self, takeoffAlt, currentLat, currentLon, *_params):
        result, message = self.validateTakeoffCommand(takeoffAlt, currentLat, currentLon)
        msg = serialize_response(request_function=VALIDATE_TAKEOFF_REQ, result=result, message=message)
        return msg
    ...
    def validateLandingHandler(self, currentLat, currentLon, *_params):
        ...
        msg = serialize_response(request_function=VALIDATE_LANDING_REQ, result=result, message=message)
        return msg


if __name__ == "__main__":
    parser = ArgumentParser(description="safetyChecker - Launch a safety checker server")
    parser.add_argument("--port", help="Port for communication between client and server", required=True)
    parser.add_argument(
        "--vehicle_config",
