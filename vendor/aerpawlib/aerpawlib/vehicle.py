...
class Vehicle:
    ...
    async def set_armed(self, value: bool):
        ...
        while self._vehicle.armed != value:
            await asyncio.sleep(_POLLING_DELAY)

    def _initialize_prearm(self, should_postarm_init):
        while self._vehicle.system_status not in ["STANDBY", "ACTIVE"]:
            time.sleep(_POLLING_DELAY)
    ...
    async def goto_coordinates(self, coordinates: util.Coordinate, tolerance: float = 2, target_heading: float = None):
        """
    ...
    async def set_velocity(self, velocity_vector: util.VectorNED, global_relative: bool = True, duration: float = None):
        """
...
class Rover(Vehicle):
    ...
    async def goto_coordinates(self, coordinates: util.Coordinate, tolerance: float = 2.1, target_heading: float = None):
        await self.await_ready_to_move()
